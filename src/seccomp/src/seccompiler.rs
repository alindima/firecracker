...
const DEFAULT_OUTPUT_FILENAME: &str = "binary_filter.out";
...
fn get_argument_values(arguments: &ArgumentsBag) -> Result<Arguments> {
    let arch_string = arguments.value_as_string("target-arch");
    ...
    let input_file = arguments.value_as_string("input-file");
    ...
    Ok(Arguments {
        ...
        output_file: arguments.value_as_string("output-file").unwrap(),
    })
}
...
#[cfg(test)]
mod tests {
    use super::compiler::{Compiler, Error as FilterFormatError, Filter, SyscallObject};
    use super::{
