...
//! [`SyscallObject`](struct.SyscallObject.html).
//!
...
pub enum Error {
    ...
    /// Erroneous SyscallObject with neither `syscall` nor `syscalls`.
    NoSyscallFields,
...
impl fmt::Display for Error {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        match *self {
            ...
            NoSyscallFields => write!(
                f,
                "Object with neither `syscall` nor `syscalls` properties."
            ),
...
impl SyscallObject {
    ...
    fn validate(&self) -> Result<()> {
        if self.syscall.is_none() && self.syscalls.is_none() {
            return Err(Error::NoSyscallFields);
        }
...
pub(crate) struct Filter {
    ...
    /// The collection of `SyscallObject`s.
            ...
            if syscall_object.is_plural() {
                ...
                for syscall in syscall_object.syscalls.as_ref().unwrap() {
                    let syscall_nr = self
                        ...
                        .ok_or_else(|| Error::SyscallName(syscall.clone(), self.arch))?;
...
mod tests {
    ...
    fn test_make_seccomp_filter() {
        let compiler = Compiler::new(std::env::consts::ARCH.try_into().unwrap());
    ...
    fn test_compile_blob() {
        ...
        let mut no_syscall_properties_filters = HashMap::new();
        no_syscall_properties_filters.insert(
            "t2".to_string(),
            Filter::new(
                SeccompAction::Trap,
                SeccompAction::Allow,
                vec![SyscallObject::new(None, None, None, None)],
            ),
        );

        assert_eq!(
            compiler.compile_blob(no_syscall_properties_filters),
            Err(Error::NoSyscallFields)
        );
